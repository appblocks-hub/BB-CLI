...
const isValidTimeZone = (time_zone) => {
  const isValid = PG_SUPPORT_TIMEZONE.includes(time_zone)
  return isValid || 'In valid time zone'
}
...
const getJobConfig = async () => {
  const schedule = await readInput({
    name: 'schedule',
    message: 'Enter the cron schedule',
    validate: isValidJobSchedule,
    default: '* * 1 * *',
  })

  const time_zone = await readInput({
    name: 'time_zone',
    message: 'Enter the time zone',
 ...
 */
const generateJobBlock = async (entry, componentName) => {
...
module.exports = { getJobConfig, isValidTimeZone, isValidJobSchedule, generateJobBlock }
